    ...
    client.connect(testAddress).onComplete(onSuccess(conn -> {
      ...
      conn.createStream((ContextInternal) vertx.getOrCreateContext(), onSuccess(stream -> {
        ...
        stream.writeHead(new HttpRequestHead(
          HttpMethod.GET, "/", MultiMap.caseInsensitiveMultiMap(), DEFAULT_HTTP_HOST_AND_PORT, "", null), false, Unpooled.EMPTY_BUFFER, false, new Http2StreamPriority(), false, null);
      }));
    }));
    await();
  }
  ...
  public void testServerConnectionClose() throws Exception {
    ...
    client.connect(testAddress).onComplete(onSuccess(conn -> {
      ...
      conn.createStream((ContextInternal) vertx.getOrCreateContext(), onSuccess(stream -> {
        ...
        stream.writeHead(new HttpRequestHead(
          HttpMethod.GET, "/", MultiMap.caseInsensitiveMultiMap(), DEFAULT_HTTP_HOST_AND_PORT, "", null), false, Unpooled.EMPTY_BUFFER, true, new Http2StreamPriority(), false, null);
      }));
    }));
    await();
  }
